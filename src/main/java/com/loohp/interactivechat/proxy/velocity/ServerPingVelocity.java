...
public class ServerPingVelocity {
    ...
    static {
        ThreadFactory factory = new ThreadFactoryBuilder().setNameFormat("InteractiveChatVelocity Async Backend Server Ping Thread #%d").build();
    ...
    public static CompletableFuture<ServerPingVelocity> getPing(ServerInfo server) {
        {
            CompletableFuture<ServerPingVelocity> future = ACTIVE_PING.get(server);
            if (future != null) {
                return future;
            }
        }
